...
import { getLocationDetailData } from '@/lib/supabase/location-detail'
...
function ConditionBadge({ value }: { value: string | null | undefined }) {
  ...
  const colour =
    ...
    value === 'damaged' ? 'bg-amber-100 text-amber-700' :
    'bg-red-100 text-red-700'
  return (
    <span className={`inline-block px-2 py-0.5 rounded text-xs font-medium capitalize ${colour}`}>
      {value}
    </span>
  )
}
    ...
    <div className={`rounded-lg border p-4 ${isPassed ? 'border-green-200 bg-green-50' : 'border-red-200 bg-red-50'}`}>
      <div className="flex items-center justify-between mb-3">
        <span className="text-sm font-semibold text-gray-700">
          Inspection #{qc.inspection_number}
        </span>
      ...
      <dl className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm mb-3">
        <div>
          ...
          <dd className="font-medium">{qc.inspector_name ?? '—'}</dd>
        </div>
        <div>
          <dt className="text-xs text-gray-500">Date</dt>
          <dd>{formatDate(qc.inspected_at)}</dd>
        </div>
        <div>
          <dt className="text-xs text-gray-500">Qty correct</dt>
          <dd><BoolCell value={qc.qty_correct} /></dd>
        </div>
        <div>
          <dt className="text-xs text-gray-500">Dimensions correct</dt>
          <dd><BoolCell value={qc.dimensions_correct} /></dd>
        </div>
        <div>
          <dt className="text-xs text-gray-500">Finish quality</dt>
          <dd><BoolCell value={qc.finish_quality_pass} /></dd>
        </div>
        <div>
          <dt className="text-xs text-gray-500">Defects present</dt>
          <dd><BoolCell value={qc.defects_present} /></dd>
        </div>
      </dl>

      {qc.overall_notes && (
        <p className="text-xs text-gray-600 italic border-t border-gray-200 pt-2 mt-2">
          &ldquo;{qc.overall_notes}&rdquo;
        </p>
      )}

      {qc.rework_required && (
        <p className="text-xs text-red-700 mt-2 font-medium">
          ⚠ Rework required{qc.rework_deadline ? ` — deadline: ${qc.rework_deadline}` : ''}
        </p>
      )}
...
export default async function LocationDetailPage({ params }: Props) {
  const data = await getLocationDetailData(params.id)
          ...
          <div className="space-y-5">
            ...
            <InfoGrid
              rows={[
                ...
                ['GPS (survey)',    (survey.gps_lat && survey.gps_lng)
                  ? `${survey.gps_lat.toFixed(6)}, ${survey.gps_lng.toFixed(6)}`
                  : null],
            ...
            <div>
              ...
              <table className="w-full text-sm border-collapse">
                ...
                <tbody>
                  <tr>
                    <td className="px-3 py-2 border border-gray-200 font-medium">Toilet</td>
                    <td className="px-3 py-2 border border-gray-200"><BoolCell value={survey.toilet_present} /></td>
                    <td className="px-3 py-2 border border-gray-200"><ConditionBadge value={survey.toilet_condition} /></td>
                  </tr>
                  <tr className="bg-gray-50">
                    <td className="px-3 py-2 border border-gray-200 font-medium">Ramp</td>
                    <td className="px-3 py-2 border border-gray-200"><BoolCell value={survey.ramp_present} /></td>
                    <td className="px-3 py-2 border border-gray-200"><ConditionBadge value={survey.ramp_condition} /></td>
                  </tr>
                  <tr>
                    <td className="px-3 py-2 border border-gray-200 font-medium">Hardware / Fittings</td>
                    <td className="px-3 py-2 border border-gray-200">—</td>
                    <td className="px-3 py-2 border border-gray-200"><ConditionBadge value={survey.hardware_condition} /></td>
                  </tr>
                </tbody>
              </table>
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                  { label: 'Tiles (sq ft)', value: survey.qty_tiles },
                  { label: 'Toilet Units',  value: survey.qty_toilet_units },
                  { label: 'Ramp Units',    value: survey.qty_ramp_units },
                  { label: 'Fitting Sets',  value: survey.qty_fittings },
                ].map(({ label, value }) => (
              ...
              {survey.qty_other && Object.keys(survey.qty_other).length > 0 && (
                <p className="mt-2 text-xs text-gray-600">
                  <span className="font-medium">Other: </span>
                  {Object.entries(survey.qty_other).map(([k, v]) => `${k}: ${v}`).join(', ')}
                </p>
              )}
            </div>
          ...
          <div className="space-y-4">
            <InfoGrid
              rows={[
                ...
                ['Status',     <span key="pjst" className={[
                  ...
                  productionJob.status === 'qc_passed'    ? 'bg-green-100 text-green-700' :
                  productionJob.status === 'qc_failed'    ? 'bg-red-100 text-red-700' :
                  productionJob.status === 'dispatched'   ? 'bg-purple-100 text-purple-700' :
                  productionJob.status === 'complete'     ? 'bg-blue-100 text-blue-700' :
                  productionJob.status === 'in_production'? 'bg-amber-100 text-amber-700' :
                  'bg-gray-100 text-gray-600',
                ].join(' ')}>
                  {productionJob.status.replace(/_/g, ' ')}
                </span>],
            ...
            <div>
              ...
              <div className="h-2.5 bg-gray-200 rounded-full overflow-hidden">
                <div
                  className={`h-full rounded-full transition-all ${productionJob.progress_pct === 100 ? 'bg-green-500' : 'bg-blue-500'}`}
                  style={{ width: `${productionJob.progress_pct}%` }}
                />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                  { label: 'Tiles (sq ft)', value: productionJob.qty_tiles },
                  { label: 'Toilet Units',  value: productionJob.qty_toilet_units },
                  { label: 'Ramp Units',    value: productionJob.qty_ramp_units },
                  { label: 'Fitting Sets',  value: productionJob.qty_fittings },
                ].map(({ label, value }) => (
                  <div key={label} className="bg-gray-50 rounded-lg p-3 text-center border border-gray-200">
                    <p className="text-2xl font-bold text-gray-900">{value ?? '—'}</p>
                    <p className="text-xs text-gray-500 mt-0.5">{label}</p>
            ...
            {productionJob.production_notes && (
              <div>
                <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1">Notes</h3>
                <p className="text-sm text-gray-700 bg-gray-50 rounded p-3 border border-gray-200 whitespace-pre-wrap">
                  {productionJob.production_notes}
                </p>
              </div>
            )}
